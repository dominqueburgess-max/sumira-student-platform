...
((SELECT id FROM lessons WHERE title='More and Less Comparisons'), 1, 'multiple_choice', 'A group of 6 and a group of 3 — which has more?', '["The group of 3", "The group of 6", "They are equal"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='More and Less Comparisons'), 2, 'short_answer', 'Make two groups of toys and tell which has less.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Number Recognition 1-5'), 1, 'multiple_choice', 'Which numeral is ''4''?', '["4", "5", "2"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Number Recognition 1-5'), 2, 'short_answer', 'Find a number between 1 and 5 somewhere in your house.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Number Recognition 6-10'), 1, 'multiple_choice', 'Which numeral is ''7''?', '["9", "7", "6"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Number Recognition 6-10'), 2, 'short_answer', 'Find and point to the number 9 somewhere.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Simple Puzzles and Spatial Awareness'), 1, 'multiple_choice', 'What can help you solve a puzzle piece?', '["Its shape and picture", "Its color only", "Guessing with eyes closed"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Simple Puzzles and Spatial Awareness'), 2, 'short_answer', 'Complete a puzzle and tell a grown-up how you did it.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Positional Words: Over, Under, Beside'), 1, 'multiple_choice', 'If a ball is over the table, where is it?', '["Above the table", "Below the table", "Next to the table"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Positional Words: Over, Under, Beside'), 2, 'short_answer', 'Place a toy under something and describe where it is.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Counting Collections'), 1, 'multiple_choice', 'Why do we touch each object as we count?', '["So we don''t skip or count twice", "To make it louder", "It doesn''t matter"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Counting Collections'), 2, 'short_answer', 'Count a collection of objects twice. Did you get the same number?', NULL, NULL),
((SELECT id FROM lessons WHERE title='One More, One Less'), 1, 'multiple_choice', 'If you have 4 toys and get 1 more, how many do you have?', '["3", "5", "4"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='One More, One Less'), 2, 'short_answer', 'Try the one more, one less game with your own toys.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Exploring 3D Shapes'), 1, 'multiple_choice', 'Which shape rolls easily?', '["A cube", "A sphere (ball)", "Neither"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Exploring 3D Shapes'), 2, 'short_answer', 'Find one round object and one boxy object at home.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Building with Blocks'), 1, 'multiple_choice', 'What happens to a block tower if the bottom is wobbly?', '["It stays strong", "It might fall", "Nothing changes"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Building with Blocks'), 2, 'short_answer', 'Build something with blocks and describe what you made.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Letter Recognition A-E'), 1, 'multiple_choice', 'Which letter comes first in the alphabet: A, C, or E?', '["A", "C", "E"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Letter Recognition A-E'), 2, 'short_answer', 'Find the letter B somewhere and point to it.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Letter Recognition F-J'), 1, 'multiple_choice', 'Which letter comes right after G?', '["F", "H", "J"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Letter Recognition F-J'), 2, 'short_answer', 'Say a word that starts with the letter J.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Letter Sound Match Games'), 1, 'multiple_choice', 'What sound does the letter ''S'' usually make?', '["/s/", "/b/", "/t/"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Letter Sound Match Games'), 2, 'short_answer', 'Match a letter to a picture and explain why they go together.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Beginning Sounds Sort'), 1, 'multiple_choice', 'Which word starts with the same sound as ''sun''?', '["Snake", "Ball", "Dog"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Beginning Sounds Sort'), 2, 'short_answer', 'Sort three objects by their beginning sound.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Story Sequencing: First, Next, Last'), 1, 'multiple_choice', 'Which word tells what happens at the very end?', '["First", "Next", "Last"]'::jsonb, 2),
((SELECT id FROM lessons WHERE title='Story Sequencing: First, Next, Last'), 2, 'short_answer', 'Retell a story using first, next, and last.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Predicting What Happens Next'), 1, 'multiple_choice', 'What does it mean to ''predict''?', '["To guess what might happen", "To read the last page first", "To close the book"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Predicting What Happens Next'), 2, 'short_answer', 'Make a prediction about a story and see if you were right.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Talking About Characters'), 1, 'multiple_choice', 'What is a ''character'' in a story?', '["A person or animal in the story", "The cover of the book", "The page number"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Talking About Characters'), 2, 'short_answer', 'Describe your favorite story character.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Drawing to Tell a Story'), 1, 'multiple_choice', 'Can pictures tell a story?', '["Yes", "No", "Only with words"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Drawing to Tell a Story'), 2, 'short_answer', 'Draw a 3-picture story and tell it to someone.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Sharing News: Show and Tell'), 1, 'multiple_choice', 'What is one thing you could share in ''show and tell''?', '["A favorite toy", "Nothing at all", "Only silence"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Sharing News: Show and Tell'), 2, 'short_answer', 'Share news about something fun that happened to you.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Putting Groups Together'), 1, 'multiple_choice', 'If you put together 2 blocks and 3 blocks, how many in all?', '["4", "5", "6"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Putting Groups Together'), 2, 'short_answer', 'Put two small groups together and count the total.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Taking Numbers Apart'), 1, 'multiple_choice', 'Which is one way to split 5 into two groups?', '["2 and 3", "5 and 5", "1 and 1"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Taking Numbers Apart'), 2, 'short_answer', 'Split 6 objects into two groups two different ways.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Decomposing Numbers to 10'), 1, 'multiple_choice', 'Which pair makes 10?', '["4 and 5", "6 and 4", "3 and 8"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Decomposing Numbers to 10'), 2, 'short_answer', 'Find two different pairs of numbers that make 10.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Making 10'), 1, 'multiple_choice', 'If you have 7, how many more make 10?', '["2", "3", "4"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Making 10'), 2, 'short_answer', 'Find how many more you need to make 10, starting from 5.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Addition Story Problems'), 1, 'multiple_choice', 'There were 3 ducks. 2 more came. How many now?', '["5", "3", "2"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Addition Story Problems'), 2, 'short_answer', 'Tell your own addition story using toys.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Subtraction Story Problems'), 1, 'multiple_choice', 'There were 6 birds. 2 flew away. How many are left?', '["4", "6", "2"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Subtraction Story Problems'), 2, 'short_answer', 'Tell your own subtraction story using toys.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Fluently Adding within 5'), 1, 'multiple_choice', 'What is 2 + 2?', '["3", "4", "5"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Fluently Adding within 5'), 2, 'short_answer', 'Solve three quick addition problems within 5.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Understanding Teen Numbers'), 1, 'multiple_choice', 'The number 14 is made of 10 and how many more?', '["3", "4", "5"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Understanding Teen Numbers'), 2, 'short_answer', 'Show the number 16 as a group of 10 plus more.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Patterns in Numbers 11-19'), 1, 'multiple_choice', 'What do all numbers from 11-19 have in common?', '["They all have a group of 10", "They are all even", "They are all less than 10"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Patterns in Numbers 11-19'), 2, 'short_answer', 'Build three different teen numbers using blocks.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Understanding Place Value'), 1, 'multiple_choice', 'In the number 42, how many tens are there?', '["2", "4", "42"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Understanding Place Value'), 2, 'short_answer', 'Build a two-digit number with blocks and describe the tens and ones.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Adding a Two-Digit and One-Digit Number'), 1, 'multiple_choice', 'What is 34 + 5?', '["39", "40", "29"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Adding a Two-Digit and One-Digit Number'), 2, 'short_answer', 'Solve 52 + 7 and explain your thinking.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Adding Two Two-Digit Numbers'), 1, 'multiple_choice', 'What is 23 + 14?', '["27", "37", "47"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Adding Two Two-Digit Numbers'), 2, 'short_answer', 'Solve 45 + 22 and show your steps.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Subtracting Multiples of 10'), 1, 'multiple_choice', 'What is 70 - 30?', '["30", "40", "50"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Subtracting Multiples of 10'), 2, 'short_answer', 'Solve 80 - 50 using drawings of tens.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Two-Step Word Problems'), 1, 'multiple_choice', '5 apples, get 3 more, then eat 2. How many are left?', '["6", "8", "10"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Two-Step Word Problems'), 2, 'short_answer', 'Solve a two-step story problem and show each step.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Doubles and Near Doubles'), 1, 'multiple_choice', 'If 5 + 5 = 10, what is 5 + 6?', '["10", "11", "12"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Doubles and Near Doubles'), 2, 'short_answer', 'Solve three doubles facts: 2+2, 6+6, and 7+7.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Skip Counting by 2s'), 1, 'multiple_choice', 'What comes next: 6, 8, 10, ___?', '["11", "12", "14"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Skip Counting by 2s'), 2, 'short_answer', 'Count by 2s from 0 to 20 to a grown-up.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Skip Counting by 5s'), 1, 'multiple_choice', 'What comes next: 15, 20, 25, ___?', '["26", "30", "35"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Skip Counting by 5s'), 2, 'short_answer', 'Count by 5s from 0 to 50.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Skip Counting by 10s'), 1, 'multiple_choice', 'What comes next: 40, 50, 60, ___?', '["61", "70", "65"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Skip Counting by 10s'), 2, 'short_answer', 'Count by 10s from 0 to 100.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Subtracting Three-Digit Numbers'), 1, 'multiple_choice', 'What is 486 - 253?', '["233", "243", "133"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Subtracting Three-Digit Numbers'), 2, 'short_answer', 'Solve 795 - 462 and show your steps.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Regrouping in Addition'), 1, 'multiple_choice', 'When do we regroup while adding?', '["When ones add up to more than 9", "Every single time", "Never"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Regrouping in Addition'), 2, 'short_answer', 'Solve 49 + 35 using regrouping and show your steps.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Regrouping in Subtraction'), 1, 'multiple_choice', 'Why might we regroup in subtraction?', '["We don''t have enough ones to subtract", "The numbers are too small", "We never need to"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Regrouping in Subtraction'), 2, 'short_answer', 'Solve 51 - 24 using regrouping and show your steps.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Solving Two-Step Word Problems'), 1, 'multiple_choice', '45 stickers, get 20 more, give away 15. How many now?', '["50", "60", "40"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Solving Two-Step Word Problems'), 2, 'short_answer', 'Solve a two-step word problem and show your steps.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Odd and Even Numbers'), 1, 'multiple_choice', 'Is 7 an odd or even number?', '["Odd", "Even", "Neither"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Odd and Even Numbers'), 2, 'short_answer', 'Test if 12 is odd or even by pairing objects.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Arrays and Repeated Addition'), 1, 'multiple_choice', '3 rows of 4 objects is the same as which addition?', '["4+4+4", "3+3+3", "4+3"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Arrays and Repeated Addition'), 2, 'short_answer', 'Build an array with your own objects and describe it.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Equal Groups'), 1, 'multiple_choice', 'If you split 12 objects into 3 equal groups, how many in each?', '["3", "4", "5"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Equal Groups'), 2, 'short_answer', 'Make equal groups with your own objects and describe them.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Money: Counting Coins and Bills'), 1, 'multiple_choice', 'How much are 2 dimes and 1 nickel worth?', '["15 cents", "20 cents", "25 cents"]'::jsonb, 2),
((SELECT id FROM lessons WHERE title='Money: Counting Coins and Bills'), 2, 'short_answer', 'Count a group of coins and find the total.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Making Change'), 1, 'multiple_choice', 'A toy costs 75 cents. You pay a dollar. How much change?', '["15 cents", "25 cents", "35 cents"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Making Change'), 2, 'short_answer', 'Solve a making-change problem of your own.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Identifying Story Characters'), 1, 'multiple_choice', 'What is a character?', '["A person or animal in a story", "The name of the book", "The last page"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Identifying Story Characters'), 2, 'short_answer', 'Name a character from a story you read.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Identifying Story Settings'), 1, 'multiple_choice', 'What is a story''s ''setting''?', '["Where and when it happens", "The main character''s name", "The book''s cover color"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Identifying Story Settings'), 2, 'short_answer', 'Describe the setting of a story you read.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Sequencing Story Events'), 1, 'multiple_choice', 'What comes first when telling a story in order?', '["The beginning", "The end", "The middle"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Sequencing Story Events'), 2, 'short_answer', 'Retell a story in order using pictures or words.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Comparing Two Characters'), 1, 'multiple_choice', 'What does it mean to ''compare'' two characters?', '["Look at how they are alike and different", "Only read about one", "Skip the story"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Comparing Two Characters'), 2, 'short_answer', 'Compare two characters you know from stories.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Making Predictions'), 1, 'multiple_choice', 'A ''prediction'' is...', '["A guess about what happens next", "The title of a book", "A character''s name"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Making Predictions'), 2, 'short_answer', 'Make a prediction in a story and check if you were right.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Connecting Text to Self'), 1, 'multiple_choice', 'What does it mean to ''connect'' a story to yourself?', '["Notice how it reminds you of your own life", "Memorize every word", "Ignore the story"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Connecting Text to Self'), 2, 'short_answer', 'Share how a story reminded you of something in your life.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Understanding Story Problems and Solutions'), 1, 'multiple_choice', 'What is a story''s ''solution''?', '["How the problem gets fixed", "The book''s title", "The first page"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Understanding Story Problems and Solutions'), 2, 'short_answer', 'Describe a problem and solution from a story.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Writing About a Favorite Character'), 1, 'multiple_choice', 'When we write an opinion, we should include a...', '["Reason", "Blank page", "Random number"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Writing About a Favorite Character'), 2, 'short_answer', 'Write or say why you like your favorite character.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Sharing Opinions About Books'), 1, 'multiple_choice', 'When sharing an opinion about a book, we should explain...', '["Why we feel that way", "Nothing else", "Only the title"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Sharing Opinions About Books'), 2, 'short_answer', 'Share your opinion about a book you read and why you feel that way.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Identifying the Main Topic'), 1, 'multiple_choice', 'What is the ''main topic'' of a nonfiction book?', '["What it is mostly about", "The last sentence", "The author''s name"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Identifying the Main Topic'), 2, 'short_answer', 'Read a nonfiction book and name its main topic.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Using Text Features in Nonfiction'), 1, 'multiple_choice', 'What is a ''heading'' in a book?', '["A title for a section", "A picture", "The very last word"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Using Text Features in Nonfiction'), 2, 'short_answer', 'Find a heading in a book and explain what it tells you.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Comparing Two Texts on the Same Topic'), 1, 'multiple_choice', 'What does it mean to compare two texts?', '["Look at how they are alike and different", "Read only one", "Ignore both"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Comparing Two Texts on the Same Topic'), 2, 'short_answer', 'Compare two books on the same topic.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Understanding Cause and Effect'), 1, 'multiple_choice', 'In ''It rained, so the ground got wet,'' what is the effect?', '["It rained", "The ground got wet", "Neither"]'::jsonb, 1),
((SELECT id FROM lessons WHERE title='Understanding Cause and Effect'), 2, 'short_answer', 'Make up a cause and effect example of your own.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Making Inferences from Pictures'), 1, 'multiple_choice', 'What can pictures in a book help us understand?', '["Feelings and details not always in the words", "Nothing useful", "Only colors"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Making Inferences from Pictures'), 2, 'short_answer', 'Look at a picture in a book and describe what you can infer from it.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Building Vocabulary from Context'), 1, 'multiple_choice', 'What can help you figure out a new word''s meaning?', '["The words around it", "The book''s cover", "Closing your eyes"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Building Vocabulary from Context'), 2, 'short_answer', 'Find a new word in a book and guess its meaning from context.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Writing a Personal Narrative'), 1, 'multiple_choice', 'A personal narrative is a story about...', '["Something that really happened to you", "A made-up dragon", "Nothing at all"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Writing a Personal Narrative'), 2, 'short_answer', 'Write 2-3 sentences about something that happened to you.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Writing an Opinion Piece'), 1, 'multiple_choice', 'An opinion piece should include a...', '["Reason for your opinion", "Random number", "Blank page"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Writing an Opinion Piece'), 2, 'short_answer', 'Write an opinion sentence about your favorite thing and why.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Using Describing Words'), 1, 'multiple_choice', 'What do describing words help us do?', '["Picture things more clearly", "Skip the sentence", "Make it shorter"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Using Describing Words'), 2, 'short_answer', 'Describe an object using three describing words.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Comparing Points of View'), 1, 'multiple_choice', 'What is ''point of view''?', '["How a character sees or feels about something", "The book''s title", "The last page"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Comparing Points of View'), 2, 'short_answer', 'Describe how two characters might see the same event differently.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Understanding Cause and Effect Relationships'), 1, 'multiple_choice', 'A chain of causes and effects is like...', '["Dominoes falling one after another", "A single still picture", "A blank page"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Understanding Cause and Effect Relationships'), 2, 'short_answer', 'Create a 3-step cause and effect chain of your own.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Making and Confirming Predictions'), 1, 'multiple_choice', 'After making a prediction, what should we do?', '["Check if it was right", "Forget about it", "Stop reading"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Making and Confirming Predictions'), 2, 'short_answer', 'Make a prediction in a story and confirm if you were right.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Summarizing a Story'), 1, 'multiple_choice', 'A summary should include...', '["Only the most important parts", "Every single word", "Nothing at all"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Summarizing a Story'), 2, 'short_answer', 'Summarize a story you read in 2-3 sentences.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Identifying Author''s Purpose'), 1, 'multiple_choice', 'Why might an author write a nonfiction book?', '["To inform readers about facts", "Only to make readers laugh", "For no reason"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Identifying Author''s Purpose'), 2, 'short_answer', 'Find a text and describe why you think the author wrote it.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Writing a Persuasive Piece'), 1, 'multiple_choice', 'A persuasive piece tries to...', '["Convince the reader of something", "Just tell a random story", "Confuse the reader"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Writing a Persuasive Piece'), 2, 'short_answer', 'Write a short persuasive paragraph about something you care about.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Writing a Research Paragraph'), 1, 'multiple_choice', 'What should a research paragraph include?', '["Facts about the topic", "Only your opinion", "Nothing at all"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Writing a Research Paragraph'), 2, 'short_answer', 'Write a short paragraph with 3 facts about a topic you researched.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Using Transition Words'), 1, 'multiple_choice', 'Which is a transition word?', '["Finally", "Elephant", "Purple"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Using Transition Words'), 2, 'short_answer', 'Write sentences about your day using transition words.', NULL, NULL),
((SELECT id FROM lessons WHERE title='Revising for Clarity'), 1, 'multiple_choice', 'What does it mean to ''revise'' your writing?', '["Reread and improve it", "Throw it away", "Never look at it again"]'::jsonb, 0),
((SELECT id FROM lessons WHERE title='Revising for Clarity'), 2, 'short_answer', 'Revise a sentence you wrote to make it clearer.', NULL, NULL);
