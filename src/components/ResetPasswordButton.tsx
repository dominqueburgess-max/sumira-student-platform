...
export function ResetPasswordButton({ studentId, studentEmail }: { studentId: number; studentEmail: string }) {
  ...
  if (result) {
    return (
      <div className="bg-sage/10 border border-sage/30 rounded-xl px-4 py-3 text-sm">
        ...
        <p className="text-xs text-warm-gray-light mt-2">This is shown once — copy it now. The student&rsquo;s old password no longer works.</p>
      </div>
    );
  }
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2 text-sm">
        <span className="text-warm-gray">Reset {studentEmail}&rsquo;s password?</span>
        <button
          onClick={handleReset}
          disabled={loading}
