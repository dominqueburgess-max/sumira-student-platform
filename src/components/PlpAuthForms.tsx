...
export function PlpSignupForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError("");
    setLoading(true);
    const res = await fetch("/api/plp-auth/signup", {
      method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full max-w-sm">
      ...
      <div className="flex gap-3">
        <label className="flex flex-col gap-1 text-sm font-semibold text-plum flex-1">
          First name
...
export function PlpLoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError("");
    setLoading(true);
    const res = await fetch("/api/plp-auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
