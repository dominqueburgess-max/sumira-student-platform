...
import { PlpLoginForm } from "@/components/PlpAuthForms";

export default async function PlpLoginPage() {
  ...
  return (
    <main className="flex-1 flex items-center justify-center bg-gradient-to-br from-plum to-plum-dark px-6 py-16">
      <div className="bg-ivory rounded-3xl card-shadow p-10 w-full max-w-md flex flex-col items-center">
        ...
        <p className="text-warm-gray text-sm mb-6 text-center">Log in to your Learning Blueprint account.</p>
        <PlpLoginForm />
        <p className="text-sm text-warm-gray mt-6">
          New here?{" "}
          <Link href="/plp/signup" className="text-terracotta-dark font-semibold">Create an account</Link>
        </p>
      </div>
    </main>
  );
